# Cluster 45 Validation Pack

## Status
Machine-generated. **Human review required.** Not an accusation or finding of wrongdoing.

## Investigation target
- Community ID: **45**
- Human anchor: **tracy jane pozo**
- Generated at: 2026-05-19 16:48:35 UTC
- Script: `scripts/build_validation_pack.py`
- Threshold: 0.9

### Data gaps / warnings
- ⚠️ dossier file not found at /app/docs/reports/dossiers/tracy-jane-pozo.md — using parquet only.

## Machine-generated triage summary
| Metric | Value |
| --- | --- |
| Priority score | 0.367107 |
| Cluster size | 13 |
| Cluster confidence | 0.9025 |
| Mean edge credibility | 0.9025 |
| Contradiction density | 0.0 |
| Anomaly score | 0.7230769230769231 |
| Isolation | 1.0 |
| Seed nodes | 4 |
| Underreportedness | 1.0 |
| Attestation density | 0.0 |

## Why this cluster ranked highly
The composite priority formula produced **0.367107**, weighted on:

- structurally strange (anomaly): 0.7231
- high-confidence: 0.9025
- highly connected: 0.4823
- underreported: 1.0

See `docs/reports/validation_queue.md` for the full ranking context.

## Community sample members (first 20)
| Name | uid | Jurisdiction | Seed? |
| --- | --- | --- | --- |
| SHERNON HOLDINGS LIMITED | icij:55026048 | mt |  |
| BELLA VISTA LIMITED | icij:55021609 | mt |  |
| icij:58016802 | icij:58016802 |  |  |
| M K MALTA LIMITED | icij:55022117 | mt |  |
| PRIMA DONNA LIMITED | icij:55020226 | mt |  |
| ENIGMA LIMITED | icij:55016371 | mt |  |
| INTERNATIONAL PARTICIPATIONS HOLDINGS LIMITED | icij:55020239 | mt |  |
| INTERNET MARKETING LIMITED | icij:55018625 | mt |  |
| MATCON INVESTMENTS LIMITED | icij:55025855 | mt |  |
| ARBOREA PROPERTIES LIMITED | icij:55023380 | mt | seed |
| RUTHWELL LIMITED | icij:55025610 | mt | seed |
| SOPHORA PROPERTIES LIMITED | icij:55022389 | mt | seed |
| ULNESS INVESTMENTS LIMITED | icij:55024977 | mt | seed |

_Total cluster size: 13_

## tracy jane pozo dossier summary
_(no dossier file)_

## Company overlap table
| Company | Normalized | In cluster? | In dossier? | Match type | Conf |
| --- | --- | --- | --- | --- | --- |
| ARBOREA PROPERTIES LIMITED | arborea properties | true | false | community_only | 1.00 |
| BELLA VISTA LIMITED | bella vista | true | false | community_only | 1.00 |
| ENIGMA LIMITED | enigma | true | false | community_only | 1.00 |
| icij:58016802 | icij 58016802 | true | false | community_only | 1.00 |
| INTERNATIONAL PARTICIPATIONS HOLDINGS LIMITED | international participations holdings | true | false | community_only | 1.00 |
| INTERNET MARKETING LIMITED | internet marketing | true | false | community_only | 1.00 |
| M K MALTA LIMITED | m k malta | true | false | community_only | 1.00 |
| MATCON INVESTMENTS LIMITED | matcon investments | true | false | community_only | 1.00 |
| PRIMA DONNA LIMITED | prima donna | true | false | community_only | 1.00 |
| RUTHWELL LIMITED | ruthwell | true | false | community_only | 1.00 |
| SHERNON HOLDINGS LIMITED | shernon holdings | true | false | community_only | 1.00 |
| SOPHORA PROPERTIES LIMITED | sophora properties | true | false | community_only | 1.00 |
| ULNESS INVESTMENTS LIMITED | ulness investments | true | false | community_only | 1.00 |

_Exact normalized-name overlap: **0** companies. Full CSV: `data/cluster_45_person_overlap.csv`._

## Person-company role evidence
| Company | Relationship | Role | Leak |
| --- | --- | --- | --- |
| SOPHORA PROPERTIES LIMITED | officer_of | secretary of | Paradise Papers - Malta corporate registry |
| ARBOREA PROPERTIES LIMITED | officer_of | secretary of | Paradise Papers - Malta corporate registry |
| ULNESS INVESTMENTS LIMITED | officer_of | secretary of | Paradise Papers - Malta corporate registry |
| RUTHWELL LIMITED | officer_of | secretary of | Paradise Papers - Malta corporate registry |
| MATCON INVESTMENTS LIMITED | officer_of | secretary of | Paradise Papers - Malta corporate registry |
| SHERNON HOLDINGS LIMITED | officer_of | secretary of | Paradise Papers - Malta corporate registry |

_6 edges total. Full CSV: `data/cluster_45_person_company_roles.csv`._

## Recurring infrastructure
### Addresses
| Address | N linked | Source |
| --- | --- | --- |
| 3/2 G. MUSCAT AZZOPARDI STREET, SLIEMA, MALTA | 12 | Paradise Papers - Malta corporate registry |

### Officers / directors
| Officer | N linked | Roles |
| --- | --- | --- |
| SOVEREIGN TRUST (MALTA) LIMITED | 11 | shareholder of |
| EDWARD ENGERER | 10 | liquidator of |
| GERALDINE FRANCES DUO | 6 | secretary of |
| SOVEREIGN DIRECTORS (T & C) LIMITED | 6 | director of;judicial representative of;legal representative of |
| TRACY JANE POZO | 6 | secretary of |
| MIDLAND INVESTMENTS LIMITED | 3 | shareholder of |
| AUGUST F SPRENG | 2 | director of;judicial representative of |
| MARK A. MIGGIANI | 2 | director of;secretary of;shareholder of |

### Agents / intermediaries
_(no rows)_

### Secretaries / service providers
_(officer rows with role containing 'secretary' or 'service' surfaced in the officers table above; not separated in the source data.)_

## Company theme classification
| Theme | Count |
| --- | --- |
| unknown | 6 |
| investment / holding / capital | 4 |
| property / real estate | 2 |
| media / marketing | 1 |

_These are **weak labels** — keyword-based heuristics, every row is flagged needs_manual_review = true._

## Graph paths
See `data/cluster_45_graph_paths.md` for the full extracted-paths report.

## External search queue
`data/cluster_45_external_search_queries.csv` contains **122** queries.
No external search has been run automatically (no safe abstraction wired). The reviewer should execute these queries by hand against the appropriate registries / search engines, or pass `--run-external-search` if/when an in-repo abstraction is added.

## Same-person evidence matrix
> **Are the Peter Kevin Perry records the same person?**

**Final verdict: Human review required.** Below is candidate evidence only — do not treat as a determination.

**Supporting evidence:**
- 2 ICIJ uids share normalized name `tracy jane pozo` (icij:56055864, icij:56055924).

**Contradictory evidence:**
- _(none)_

**Missing evidence:**
- country field missing in ICIJ persons rows.
- date-of-birth not available in icij_persons; cannot compare DOB.
- no canonical home-address field for persons; cannot compare residential address.

## Ordinary vs unusual analysis
| Indicator | Value | Interpretation |
| --- | --- | --- |
| Cluster size | 13 |  |
| Officer reuse rate | 0.923 | high reuse = corporate-services hub |
| Address reuse rate | 0.923 | high reuse = shared registered office |
| Intermediary reuse rate | 0.0 | high reuse = shared agent |
| Top theme | unknown | share=0.462 |
| Anomaly score | 0.7230769230769231 |  |
| Isolation | 1.0 | 1.0 = no cross-cluster bridges |
| Underreportedness | 1.0 | 1.0 = no name overlap with formal registries |
| Contradiction density | 0.0 |  |
| # repeated addresses | 1 |  |
| # repeated officers | 8 |  |
| # repeated intermediaries | 0 |  |

> **TODO:** percentile ranks against a cluster baseline have not been computed (no per-cluster baseline parquet present). Raw values only above.

## Contradictory or missing evidence
- ICIJ records do not include date-of-birth for persons; same-person claims cannot be tested on DOB.
- No web-corroboration is wired (dossier records 0 hits as of 2026-05-16).
- Address fields on most cluster member entity rows are null; address-bridge analysis depends on `registered_address` edges.

## Human review checklist
- [ ] Confirm whether the Peter Kevin Perry records refer to the same person
- [ ] Verify each overlapping company in source records
- [ ] Verify roles and dates
- [ ] Check external registry records (MFSA, Companies House, etc.)
- [ ] Check litigation / insolvency / sanctions / procurement / property / gaming / yacht records
- [ ] Determine whether this is ordinary corporate-services plumbing or unusual network structure
- [ ] Draft cautious narrative if evidence supports it

## Preliminary human verdict
Pending.

## Publication risk notes
- No allegations should be made without independent corroboration.
- Same-name does not imply same-person.
- Corporate-services hubs in Malta legitimately serve hundreds of unrelated clients; recurring infrastructure is a starting point, not an indictment.
- Preserve provenance (source_label, leak name, uid) for every claim carried into any published narrative.
