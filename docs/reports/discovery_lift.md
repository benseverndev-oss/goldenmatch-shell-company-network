# Discovery-lift benchmark

_Generated 2026-05-16 23:49 UTC by `scripts/render_discovery_lift.py` from a Railway-side
...
|---|---|---:|
| **B1** Naive case-fold pairwise | Lowercase + whitespace-strip only. No legal-suffix removal, no ASCII fold. | 22,695 |
| **B2** Goldenmatch-normalized | Adds `normalize_company_name` — legal-suffix strip, ASCII fold, tokenize. | 25,247 |
| **B3** + Rare-filter | Drops common-name explosions (`max_per_source ≤ 2`, `n_tokens ≥ 3`). | 3,838 |
| **B4** + Dossier pipeline | B3 seeds enriched with 2-hop ICIJ graph + sanctions overlay. | 50 |

## Tier deltas (incremental gain)

| Step | Delta | Reading |
|---|---:|---|
| B1 → B2 | +2,552 | Anchors rescued by goldenmatch's normalization step (legal-suffix strip + ASCII fold). The recall lift. |
| B2 → B3 | -21,409 | Anchors *cut* by the rare-filter. The precision move — common-name explosions removed. Negative by design. |
| B3 → B4 | -3,788 | B4 is the top-N sample (default 50) of B3 enriched with the 2-hop graph walk. The negative delta is the top-N cap, not a real precision loss. The lift here is qualitative (linked companies + addresses + sanctions adjacency), not numerical. Bump `--top-n` to operate at B3's full scale. |

**Anchors with investigative signal at B4:**
46 of 50 dossier seeds have ≥1 linked company surfaced by the ICIJ 2-hop walk
(0 with sanctions adjacency hit).

## Reachability cross-tabulation

Of the **union of all tiers' multi-source anchors** (25,247 distinct names total):

- **0** are reachable only by the naive baseline. These are likely *spurious* — names that happen to match across sources without the normalize step but that goldenmatch deliberately rejects (e.g. variants where one side has an unstripped legal suffix that B1 keeps and B2 strips, creating a difference).
- **1,641** are reachable at B2 but not B1. These are the *recall lift* — real cross-source overlaps that naive case-folding misses because of legal-suffix differences or ASCII-fold issues.
- **7** of the 50 dossier seeds would be **invisible to a naive baseline**. That's the headline lift number for the dossier pipeline specifically.
