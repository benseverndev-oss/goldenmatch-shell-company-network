...
def main() -> None:
    ...
    for r in top:
        ...
        print(
            f"  {r['__match_score__']:>5}  [{r['target_source'][:4]}|{t_jur}] {t_name!r:42s}  vs  [{r['ref_source'][:4]}|{r_jur}] {r_name!r}"
        )
    ...
    if icij_matches:
        ...
        for r in sample:
            print(
                f"  {r['__match_score__']:>5}  [icij|{r.get('target_jurisdiction', '')}] {(r['target_name'] or '')[:45]!r}  ->  [gleif|{r.get('ref_jurisdiction', '')}] {(r['ref_name'] or '')[:45]!r}"
            )
