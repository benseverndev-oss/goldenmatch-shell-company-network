"""Ingest an OpenOwnership UK BODS parquet bundle.
    ...
    uv run python scripts/ingest_uk_bods.py \\
        --input data/raw/openownership/uk_bods.zip

Downloads to ``/data/raw/openownership/uk_bods.zip`` via ``/fetch-url``
first (or expects the file already to be there). Writes:
...
def main(
    ...
    out_dir: Path = typer.Option(
        INTERIM_DIR,
        "--out-dir",
        help="Where to write the two interim parquets.",
    ),
