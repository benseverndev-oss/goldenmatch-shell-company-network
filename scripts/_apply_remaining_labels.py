...
V2_MARGINAL: dict[str, tuple[str, str]] = {
    ...
    "P0102": (
        "match",
        "Service Immobiliere/Immobilier et Gestion SAS — French SAS, spelling variant of one entity.",
    ),
    "P0103": (
        "no_match",
        "Golden State Re vs Golden State Re II — different reinsurance vehicles.",
    ),
    ...
    "P0106": (
        "unsure",
        "Certares GBT Holdings vs Certares Holdings LLC — possibly related Certares group entities, but different legal forms.",
    ),
    "P0107": ("match", "Aplus Assets Worldwide Ltd — identical."),
    "P0108": (
        "match",
        "MALTA UNIVERSITY HOLDING COMPANY LIMITED vs Malta University Holding Co Ltd — same entity, suffix expansion only.",
    ),
    "P0109": (
        "match",
        "The Hankey No 1 Settlement vs THE HANKEY NO1 SETTLEMENT — same trust, only spacing differs.",
    ),
    "P0110": (
        "unsure",
        "Atlas Senior Loan Fund III, Ltd. Trust vs Ltd — Trust label may mean a separate trust vehicle for the same CLO.",
    ),
    ...
    "P0115": (
        "unsure",
        "ALF. MIZZI & SONS (HR) LTD vs ALF. MIZZI & SONS LTD — (HR) suggests an HR-services subsidiary distinct from the parent.",
    ),
    "P0116": ("match", "PERSISTENT PETROCHEM PRIVATE LIMITED — identical."),
    "P0117": (
        "no_match",
        "IVM 041 vs IVM 047 PURPOSE TRUST — different sub-trusts in the IVM series.",
    ),
    "P0118": (
        "unsure",
        "Shipstern Global Opportunities Fund vs Master Fund — master/feeder vehicles can be distinct legal entities or share LEIs depending on structure.",
    ),
    "P0119": ("no_match", "PA Grand Opportunity VI Limited vs main — different sub-fund."),
    "P0120": (
        "unsure",
        "Brookside Mill CLO Ltd. Trust vs Ltd — Trust label may indicate distinct trust vehicle.",
    ),
    "P0121": ("match", "IRON WILL INVESTMENT LTD — identical."),
    "P0122": (
        "match",
        "First Crockford Advisors/Advisers Ltd — same entity, US/UK spelling variant.",
    ),
    ...
    "P0127": (
        "no_match",
        "Morgan Stanley Reinsurance Alpha Ltd vs main MS Reinsurance Ltd — distinct sub-vehicle.",
    ),
    "P0128": (
        "no_match",
        "MEDSEA INVESTMENTS LIMITED vs Medsea Investments Two Limited — distinct.",
    ),
    "P0129": ("match", "PICOTIN HOLDINGS LIMITED — identical."),
    "P0130": (
        "no_match",
        "Mediterranean Trade vs Mediterranean Travel Services — different industries.",
    ),
    "P0131": (
        "unsure",
        "Bridge Electronic Trading Ltd vs Bridge Electronic Trading (BVI) Ltd — (BVI) suffix may denote BVI subsidiary distinct from parent.",
    ),
    "P0132": (
        "match",
        "SUEX OTC s.r.o. — identical Czech sro (note: SUEX was sanctioned by US Treasury in 2021 for crypto-money-laundering).",
    ),
    "P0133": ("match", "ASIA FORTUNE GROUP LIMITED — identical."),
    "P0134": (
        "unsure",
        "Investcorp Aerwulf Credit Opportunities Fund vs Master Fund — master/feeder pair, may share or have distinct LEIs.",
    ),
    "P0135": (
        "unsure",
        "KB HOLDING LIMITED vs KB HOLDINGS LTD — Holding/Holdings could be same entity or separate.",
    ),
    "P0136": ("no_match", "INTERTRUST TRUSTEES II vs Intertrust Trustees — distinct."),
    "P0137": (
        "no_match",
        "ADVANTECH HOLDINGS LIMITED vs Advantec Holding S.A. — distinct (different spelling, different legal form).",
    ),
    "P0138": (
        "unsure",
        "Atlas Senior Loan Fund IV, Ltd. Trust vs Ltd — Trust label may indicate separate trust vehicle.",
    ),
    ...
    "P0145": (
        "unsure",
        "GY AVIATION LEASE (MALTA) LIMITED vs GY Aviation Lease (Malta) Co. Limited — Co. suffix variant; could be same entity or sister.",
    ),
    "P0146": (
        "no_match",
        "CORPORACION CIMEX, S.A. vs CORPORACION COEX, INC. — distinct entities (note: CIMEX is a sanctioned Cuban entity).",
    ),
    "P0147": (
        "no_match",
        "PSAM WorldArb SRI Fund vs PSAM WorldArb Fund — SRI is a distinct ESG sub-fund.",
    ),
    ...
    "P0153": (
        "unsure",
        "Golub Capital Partners CLO 16 vs 16(M)-R — (M)-R indicates a refinanced master tranche; may be the same underlying CLO or a distinct refinanced vehicle.",
    ),
    ...
    "P0160": (
        "no_match",
        "Windsor Properties (6) Limited vs Windsor Properties Limited — distinct sub-vehicles in series.",
    ),
    ...
    "P0164": (
        "no_match",
        "PAMPLONA CREDIT OPPORTUNITIES INVESTMENTS HOLDING LIMITED vs INVESTMENTS LIMITED — holdco vs underlying portfolio entity.",
    ),
    "P0165": (
        "unsure",
        "ASSIKURA INSURANCE BROKERS LTD vs PCC LTD — PCC (Protected Cell Company) is a distinct legal form for Malta; could be same entity restructured or two distinct entities.",
    ),
    ...
    "P0178": (
        "unsure",
        "Phoenix Real Estate Fund GP vs Fund (T) L.P. — GP and Fund are distinct legal vehicles in same fund structure.",
    ),
    "P0179": ("match", "TAIWIN INTERNATIONAL LTD — identical."),
    "P0180": ("no_match", "Orion Co-Investments I vs III — distinct vintages."),
    "P0181": (
        "no_match",
        "Aeolus Property Catastrophe Spire Fund LP vs Sphaera Fund Ltd — distinct sub-funds (different name + different legal form).",
    ),
    ...
    "P0185": (
        "match",
        "Brookfield Renewable Energy Partners L.P. vs Brookfield Renewable Partners L.P. — same entity; renamed in 2016 (publicly documented BEP rebrand).",
    ),
    "P0186": ("no_match", "JMP CLO II vs III (R) — different vintages."),
    "P0187": ("match", "PowerFinance International Ltd — identical."),
    "P0188": (
        "unsure",
        "Invesco Active Multi-Sector Credit Fund (Cayman) vs main — (Cayman) is a regional flag; likely same fund but parent group runs multi-jurisdictional vehicles.",
    ),
    ...
    "P0194": (
        "unsure",
        "AEROSPACE INSPECTION TRAINING MALTA LIMITED vs HOLDING LTD — operating co vs holdco in same group.",
    ),
    ...
    "P0199": (
        "no_match",
        "Manulife (International) P & C Limited vs MANULIFE (INTERNATIONAL) LIMITED — P&C is a distinct property-and-casualty subsidiary.",
    ),
...
PERFECT_SANITY: dict[str, tuple[str, str]] = {
    pid: (
        "match",
        "Identical legal name (case/punctuation only); perfect-band sanity-check passes.",
    )
...
V2_BORDERLINE: dict[str, tuple[str, str]] = {
    "P0251": (
        "match",
        "NOVA RENEWABLE ENERGY (NOVARE) FUND SICAV P.L.C. vs PLC — same entity, dot variant.",
    ),
    "P0252": ("no_match", "PA Grand Opportunity III vs main — distinct sub-vehicle."),
    "P0253": (
        "no_match",
        "STAMFORD MANAGEMENT vs Stamford Land Management — distinct (Stamford Land is a publicly-listed Singapore real-estate co).",
    ),
    ...
    "P0259": (
        "match",
        "Golub Capital Partners CLO Holdings Ltd. — identical (comma variant only).",
    ),
    "P0260": (
        "unsure",
        "Zen Shipping & Port India vs ZEN SHIPPING & PORTS INDIA — Port/Ports may be a typo or distinct registration.",
    ),
    "P0261": (
        "unsure",
        "Golub Capital Partners CLO 22(B) vs 22(B)-R — refinanced version may share or have distinct LEIs.",
    ),
    ...
    "P0268": (
        "unsure",
        "Scopia LB International Master Fund vs Scopia Long International Master Fund — LB may abbreviate Long Bias=Long; could be renamed same entity.",
    ),
    "P0269": ("unsure", "Madison Park Funding VIII Trust vs Ltd — Trust label."),
    "P0270": (
        "unsure",
        "VistaJet Malta Aircraft Eleven Limited vs Eleven MAPCC Limited — MAPCC suffix changes legal form.",
    ),
    "P0271": (
        "no_match",
        "NATURA DEVELOPMENT II vs NATURA DEVELOPMENT N.V. — distinct (same as P0154 — appears twice).",
    ),
    "P0272": (
        "match",
        "Patricia Jane Roberts Zagal Settlement vs DE Zagal Settlement — same person trust, missing 'de' particle.",
    ),
    ...
    "P0278": (
        "match",
        "Kompas Kimyevi Maddeler Pazarlama Ticaret ve Sanayi A.S. — identical Turkish name (case differs only; the ?'s are encoding artifacts of Turkish characters).",
    ),
    ...
    "P0281": (
        "no_match",
        "Mapeley Beta Acquisition Co (5) vs (1) — distinct sub-vehicles in series.",
    ),
    "P0282": (
        "match",
        "Mapeley Beta Acquistion Co (1) vs ACQUISITION CO (1) — same entity, typo in one source.",
    ),
    "P0283": (
        "match",
        "SM Managed Futures Ltd vs SM Managed Futures — same, one missing the Ltd suffix.",
    ),
    ...
    "P0286": (
        "match",
        "Invesco Senior Secured Bank Loan Fund Japan (JPY-Hedged) — same fund, hyphen variant in name.",
    ),
    ...
    "P0289": (
        "no_match",
        "Mapeley Beta Acquisition Co (4) vs (1) — distinct sub-vehicles in series.",
    ),
    ...
    "P0295": (
        "match",
        "Credit Suisse Long/Short Equity Strategy Fund Ltd. — identical (one missing trailing dot).",
    ),
    "P0296": ("no_match", "PA Grand Opportunity IX vs main — distinct sub-vehicle."),
    "P0297": (
        "unsure",
        "S3 Global Multi-Strategy Fund Ltd vs S3 Global Multi-Strategy Master Fund Ltd — feeder/master pair.",
    ),
    ...
    "P0300": (
        "no_match",
        "Mapeley Beta Acquisition Co (6) vs (1) — distinct sub-vehicles in series.",
    ),
}
